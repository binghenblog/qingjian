...
- **📝 知识库 / 笔记**：Markdown 编辑、文件夹分组、`#标签`、全文搜索（关键字高亮）、`[[双链]]` 渲染、命令面板内直接搜索笔记；删除后支持撤销恢复。
- **✅ 待办 / 任务**：
  ...
  - **📅 日程**已并入待办，统一为单一清单。
- **📦 生活模块（v0.3.0 新增）**：
  - **💪 健身 Fitness**：记录训练动作与组数，追踪健身进度。
  - **💰 记账 Ledger**：日常收支记录与分类统计。
  - **💞 纪念日 Anniversaries**：重要日期提醒与倒计时。
  - **📖 记好句 Quotes**：收藏喜欢的句子，随时回看。
- **🧭 左侧抽屉导航 + ⌘ 命令面板**：`⌘K` / `Ctrl+K` 全局唤起，搜索与操作直达；核心页面支持 `Alt+1~5` 快捷键直达；导航栏改为左侧抽屉式，点开时右侧内容自动收窄。
- **🤖 AI 助手**：
  - 双通道：**云端（OpenAI 兼容）** + **本地（Ollama）**，统一接口、随时切换。
  - 流式输出（打字机效果）、可中止。
  - 可读取本地**笔记 / 待办上下文**对话（如「就这篇笔记与 AI 讨论」「本周总结」「规划待办」）。
...
| M5 跨端打包 | tauri-action 出 exe / apk，GitHub Release | 🚧 进行中（v0.3.0 起支持 Windows / Linux 自动出包，apk / macOS 待细化） |
