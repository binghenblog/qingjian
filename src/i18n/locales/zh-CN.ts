...
export default {
  ...
  settings: {
    ...
    keySecureHint: '密钥已由系统凭据库加密保管（Windows 凭据管理器 / macOS 钥匙串 / Linux Secret Service），仅本机当前用户可读。',
    keySecureDesc: '桌面版密钥不再进浏览器存储：请求时由 Rust 后端直接从系统凭据库读取，前端不接触明文 Key，显著降低窃取风险。',
    keyHint: '密钥仅保存在本机浏览器，默认只在当前会话有效。纯 Web 端直连云端可能受 CORS 限制。',
    keyWarning: '安全提醒：当前 Web 端密钥以明文保存在浏览器存储中，同源任意脚本或 XSS 攻击都可能窃取。请勿在不可信环境输入真实密钥；建议使用桌面版，密钥将由系统凭据库加密保管。',
    exportBtn: '导出备份',
    importMerge: '导入（合并）',
    importReplace: '导入（覆盖）',
    reloadBtn: '刷新页面载入新数据',
    ...
    shortcutEnable: '启用全局快捷键',
    shortcutEnableHint: '开启后支持 Alt+1~5 切换页面、Ctrl/Cmd+K 呼出命令面板、Ctrl/Cmd+N 快速新建任务',
  ...
  notes: {
    ...
    emptyTitle: '笔记已保存在本地',
    emptyHint: '笔记已存在本地浏览器，刷新不会丢失。',
    emptyHintSub: '后续版本会支持 Obsidian 库互通，双链、标签与元数据也会同步。',
    noteLabel: '笔记',
    tabNotes: '笔记',
  },
  todos: {
    ...
    enterHint: '在输入框按 Enter 即可快速新建任务',
    catExists: '分类已存在',
  ...
  ai: {
    ...
    emptyHint: '支持本地 Ollama 与云端 OpenAI 兼容双通道，流式输出。',
    emptyHintSub: '默认走本地通道，请先安装并启动 Ollama。',
  ...
  quote: {
    ...
    save: '保存',
    cancel: '取消',
    empty: '还没有好句，点击上方「记好句」收集你想收藏的句子',
    textRequired: '句子不能为空',
    added: '已保存好句',
