// English (US) locale — mirrors zh-CN key structure.
// Add a new language by appending a same-named locale under messages in i18n/index.ts.
export default {
  app: {
    name: 'QingJian',
    tagline: 'Personal Workbench',
    footer: 'Local-first · Your data is yours to control',
    version: 'v0.6.0'
  },
  kbd: {
    search: 'Open search / command palette',
    newTask: 'Quick new task'
  },
  common: {
    confirm: 'Confirm',
    cancel: 'Cancel',
    delete: 'Delete',
    undo: 'Undo',
    add: 'Add',
    edit: 'Edit',
    save: 'Save'
  },
  nav: {
    dashboard: 'Today',
    notes: 'Notes',
    todos: 'Schedule',
    ledger: 'Ledger',
    fitness: 'Fitness',
    anniversaries: 'Anniversaries',
    ai: 'AI Assistant',
    settings: 'Settings',
    search: 'Quick search',
    searchAria: 'Quick search (Ctrl/Cmd + K)',
    toggleAria: 'Open / close navigation menu',
    more: 'More',
    back: 'Back',
    forward: 'Forward',
    close: 'Close'
  },
  dashboard: {
    greetingNight: 'Late night',
    greetingMorning: 'Good morning',
    greetingAfternoon: 'Good afternoon',
    greetingEvening: 'Good evening',
    newWork: 'New work',
    logProgress: 'Log progress',
    heroHint: 'Today, just record what truly matters',
    heroTitle: 'Compile chaos into order.',
    heroSub: 'Every entry distills your tasks, notes and everyday moments.',
    heroCta: 'Log what you just did',
    heroDismiss: 'Dismiss banner',
    todayPush: 'Push forward today',
    allTodos: 'All tasks',
    newTask: 'New task',
    doneOf: '{done} / {total} done',
    yesterdayMissed: '{n} left from yesterday',
    weekProgress: 'This week’s progress',
    recent7: 'Last 7 days',
    todayAdded: 'Added today',
    weekDone: 'Done this week',
    completionRate: 'Completion rate',
    emptyDone: 'All tasks done. Nicely done 🎉',
    emptyNone: 'No tasks yet. Add your first from the top-right.',
    featureNotes: 'Notes',
    featureNotesDesc: 'Local-first knowledge base',
    featureTodos: 'Tasks',
    featureTodosDesc: 'GTD-style task management',
    featureAi: 'AI Assistant',
    featureAiDesc: 'Cloud / Local dual channel',
    emptyDoneTitle: '🎉 All tasks done for today!',
    emptyDoneHint: 'Record what you gained today, or plan for tomorrow.',
    recTodayGain: 'Record today’s gains',
    planTomorrow: 'Plan tomorrow'
  },
  about: {
    desc: '{version} · Lightweight · Modern · Open-source · Local-first · MIT License'
  },
  settings: {
    title: 'Settings',
    profile: 'Profile',
    profileHint: 'Name shown in the dashboard greeting. Stored locally only.',
    namePlaceholder: 'Enter your name, e.g. Binghen',
    appearance: 'Appearance',
    appearanceHint: 'Choose the interface theme mode',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'Follow system',
    themeAria: 'Theme mode',
    shortcutCmdPalette: 'Command palette · global search',
    shortcutDashboard: 'Dashboard',
    shortcutNotes: 'Notes',
    shortcutTodos: 'Tasks',
    shortcutAi: 'AI assistant',
    shortcutSettings: 'Settings',
    langTitle: 'Language',
    langHint: 'Interface display language',
    langAria: 'Interface language',
    channelLocal: 'Local Ollama',
    channelCloud: 'Cloud-compatible',
    aiTitle: 'AI Channel',
    aiHint: 'Choose a local or cloud model; config is stored locally',
    dataTitle: 'Data & Storage',
    dataHint: 'Local data: {notes} notes · {todos} tasks. Backups are JSON files, restorable across devices.',
    fieldAddress: 'Endpoint',
    fieldModel: 'Model',
    fieldApiKey: 'API Key',
    rememberKey: 'Remember key on this device (writes to localStorage; if off, cleared when browser closes)',
    keySecureHint: 'The key is stored encrypted in your OS credential store (Windows Credential Manager / macOS Keychain / Linux Secret Service), readable only by the current user.',
    keySecureDesc: 'On the desktop build the key no longer touches browser storage: the Rust backend reads it directly from the OS credential store on each request, so the frontend never handles the plaintext key.',
    keyHint: 'The key is stored only in your local browser and is valid for the current session by default. A pure web client connecting to the cloud may hit CORS limits.',
    keyWarning: 'Security notice: on the Web build the key is stored in plaintext in browser storage, so any same-origin script or XSS can steal it. Do not enter a real key in untrusted environments; use the desktop build, where the key is kept encrypted by the OS credential store.',
    exportBtn: 'Export backup',
    importMerge: 'Import (merge)',
    importReplace: 'Import (replace)',
    reloadBtn: 'Reload page to load new data',
    backupHint: 'Merge: dedupe by id, keep the newer version of the same note; Replace: clear current data then restore the whole backup (asks for confirmation). Backups exclude the API Key.',
    importReplaceTitle: 'Replace import',
    importReplaceMsg: 'Replace import will clear all current notes and tasks and replace them with the backup. Continue?',
    exported: 'Backup downloaded (API Key excluded)',
    exportFailed: 'Export failed: {msg}',
    importSuccess: 'Imported: {notes} notes, {todos} tasks ({mode}). Data reloaded automatically.',
    importSuccessManual: 'Imported: {notes} notes, {todos} tasks. Click the button on the right to reload the new data.',
    importFailed: 'Import failed: {msg}',
    shortcutTitle: 'Shortcuts',
    shortcutHint: 'Jump anywhere, anytime',
    shortcutEnable: 'Enable global shortcuts',
    shortcutEnableHint: 'When enabled: Alt+1~5 to switch pages, Ctrl/Cmd+K to open command palette, Ctrl/Cmd+N to quickly add a task',
    ledgerSummary: 'Ledger summary',
    ledgerSummaryHint: 'Show this week and month totals at the top of the Ledger page',
    ledgerSummaryToggle: 'Show weekly / monthly totals',
    aiEntry: 'AI assistant entry',
    aiEntryHint: 'AI features are not in development yet; hide the entry from the sidebar and dashboard',
    aiEntryToggle: 'Show AI entry in navigation & dashboard',
    banner: 'Home banner',
    bannerHint: 'Restore the dismissed Home banner from here',
    restoreBanner: 'Restore banner',
    bannerRestored: 'Banner restored'
  },
  notes: {
    dbError: '⚠️ Local database unavailable: {err}',
    searchPlaceholder: 'Search titles, content, tags…',
    clearSearch: 'Clear search',
    newNote: 'New note',
    folderAll: 'All',
    folderUncategorized: 'Uncategorized',
    folderNamePlaceholder: 'Name',
    newFolder: 'New folder',
    deleteFolder: 'Delete folder',
    deleteFolderTitle: 'Delete folder',
    deleteFolderMsg: 'Delete folder "{name}"? Its notes will move to "Uncategorized"',
    deleteNoteTitle: 'Delete note',
    deleteNoteMsg: 'Delete note "{title}"?',
    deleted: 'Note "{title}" deleted',
    searchCount: 'Searching {total} notes, {matched} matched',
    blankNote: 'Blank note',
    emptyNoMatch: 'No matching notes',
    emptyFolder: 'This folder is empty',
    emptyNone: 'No notes yet. Tap + at top-right to create one',
    untitled: 'Untitled note',
    modeAria: 'Edit mode',
    moveToFolder: 'Move to folder',
    modeEdit: 'Edit',
    modeSplit: 'Split',
    modePreview: 'Preview',
    deleteNote: 'Delete note',
    removeTag: 'Remove tag {tag}',
    addTagPlaceholder: '+ tag',
    editorPlaceholder: 'Start writing in Markdown…',
    emptyTitle: 'Your notes are saved locally',
    emptyHint: 'Notes are stored in this browser and won\'t be lost on refresh.',
    emptyHintSub: 'A future update will connect Obsidian vaults for bidirectional links, tags and metadata.',
    noteLabel: 'Note',
    tabNotes: 'Notes'
  },
  todos: {
    title: 'Schedule & Tasks',
    deleteCategory: 'Delete category',
    deleteCategoryTitle: 'Delete category',
    deleteCategoryMsg: 'Delete category "{cat}"? Its tasks will move to "Life"',
    catNamePlaceholder: 'Category name',
    confirm: 'Confirm',
    custom: 'Custom',
    missedBanner: '{n} daily tasks were missed yesterday — catch up today 💪',
    clearBanner: 'All daily tasks done yesterday. On a roll ✨',
    todayProgress: 'Today’s progress',
    doneOf: '{done} / {total} done',
    catProgress: '"{cat}" progress',
    addDailyPlaceholder: 'Add a daily task (auto-resets daily), press Enter…',
    addCatPlaceholder: 'Add to "{cat}", press Enter…',
    add: 'Add',
    addSchedule: 'Add schedule',
    priorityAria: 'Priority',
    priorityLabel: 'Priority',
    priorityHigh: 'High',
    priorityMedium: 'Medium',
    priorityLow: 'Low',
    tagPlaceholder: 'Tag (optional), e.g. reading / fitness',
    completeTask: 'Complete task {title}',
    streakDays: '{n}d',
    deleteTask: 'Delete task',
    emptyDaily: 'No daily tasks yet. Add a habit to check in (e.g. read 30 min each morning)',
    emptyCat: '"{cat}" has no tasks yet. Add the first one above',
    dailyHint: 'Daily tasks auto-reset at midnight; completion history is kept for streak stats',
    enterHint: 'Press Enter in the input to add a task quickly',
    catExists: 'Category already exists',
    catNameHint: 'Name must be 1-8 characters'
  },
  ai: {
    title: 'AI Assistant',
    discussNote: 'Discuss with AI',
    channelPrefix: 'Channel: ',
    clearChat: 'Clear chat',
    goToSettings: 'Go to settings to switch',
    sessions: 'Sessions',
    newChat: 'New chat',
    untitledChat: 'Untitled chat',
    noChats: 'No chats yet',
    renameChat: 'Rename',
    deleteChat: 'Delete',
    deleteChatTitle: 'Delete chat',
    deleteChatMsg: 'Delete chat "{title}"? Its messages cannot be recovered',
    emptyHint: 'Supports local Ollama and cloud OpenAI-compatible channels with streaming output.',
    emptyHintSub: 'Defaults to local; install and start Ollama first.',
    placeholderNeedsKey: 'Cloud channel needs an API Key set in Settings',
    placeholder: 'Say something… (Enter to send, Shift+Enter for newline)',
    stop: 'Stop generating',
    send: 'Send',
    channelLocal: 'Local Ollama',
    channelCloud: 'Cloud',
    generating: 'Generating reply…',
    stopped: '(stopped)',
    stoppedStatus: 'Generation stopped',
    emptyModelResponse: '(model returned nothing)',
    errorStatus: 'AI reply error',
    errorFallback: 'Request failed',
    errorBody: '⚠️ {msg}\n\nIf using the cloud channel, a pure web client may hit CORS limits; the desktop build routes through a local backend to resolve this.',
    cmd: {
      actionKind: 'AI',
      summarizeWeek: "Summarize this week's notes",
      summarizeWeekTitle: "This Week's Notes Summary",
      summarizeWeekInstr: 'Summarize the notes above in bullet points, extracting key information and actionable items.',
      planTodos: 'Turn my todos into a plan',
      planTodosTitle: 'Todo Plan',
      planTodosInstr: 'Based on the todo list above, help me make a reasonable execution plan (by priority and schedule).',
      notesPreamble: "You are a local personal assistant. The following are some of the user's notes; answer based on this content.",
      todosPreamble: "You are a local personal assistant. The following is the user's todo list; answer based on it.",
      notePreamble: "You are a local personal assistant. The following is one of the user's notes; answer based on it.",
      discussNoteInstr: 'Read this note, point out its core ideas and possible extensions, or help me discuss around it.',
      noNotesThisWeek: 'No notes updated this week',
      noPendingTodos: 'No pending todos'
    }
  },
  palette: {
    title: 'Command palette',
    searchPlaceholder: 'Search pages, note content…',
    noteKind: 'Note',
    noResult: 'No matching results'
  },
  // New module namespaces (placeholder titles; detailed copy added per module phase)
  ledger: {
    title: 'Ledger',
    todayIncome: 'Today income',
    todayExpense: 'Today expense',
    weekIncome: 'Week income',
    weekExpense: 'Week expense',
    monthIncome: 'Month income',
    monthExpense: 'Month expense',
    addTitle: 'Add an entry',
    type: 'Type',
    typeIncome: 'Income',
    typeExpense: 'Expense',
    category: 'Category',
    amount: 'Amount',
    amountPlaceholder: '0.00',
    date: 'Date',
    note: 'Note',
    notePlaceholder: 'optional',
    save: 'Save',
    saved: 'Saved',
    records: 'Transactions',
    viewAll: 'View all',
    viewRecent: 'Last 3 days only',
    empty: 'No entries yet. Tap "Add an entry" above to start tracking.',
    deleteTitle: 'Delete entry',
    deleteMsg: 'Delete this "{category}" entry?',
    deleteAria: 'Delete entry',
    pickCategory: 'Please pick a category',
    invalidAmount: 'Amount must be greater than 0'
  },
  fitness: {
    title: 'Fitness',
    needHeight: 'First-time setup: enter your height',
    editHeight: 'Edit height',
    needHeightHint: 'Used to calculate BMI; stored only on this device',
    height: 'Height (cm)',
    cm: 'cm',
    heightPlaceholder: 'e.g. 175',
    heightSaved: 'Height saved',
    todayMinutes: 'Today',
    minutes: 'min',
    monthMinutes: 'This month',
    bmi: 'BMI',
    bmiThin: 'Underweight',
    bmiNormal: 'Normal',
    bmiOver: 'Overweight',
    bmiObese: 'Obese',
    currentWeight: 'Current weight',
    weightUnit: 'kg',
    addRecord: 'Add record',
    addWorkout: 'Add workout',
    workoutType: 'Type',
    workoutTypePlaceholder: 'e.g. Running',
    duration: 'Duration',
    durationPlaceholder: 'e.g. 30',
    date: 'Date',
    note: 'Note',
    notePlaceholder: 'optional',
    workouts: 'Workouts',
    emptyWorkouts: 'No workouts yet',
    addWeight: 'Log weight',
    weight: 'Weight',
    weightPlaceholder: 'e.g. 65.5',
    weights: 'Weight log',
    emptyWeights: 'No weight records yet',
    trend: 'Recent weight trend',
    deleteTitle: 'Delete record',
    deleteWorkoutMsg: 'Delete this workout?',
    deleteWeightMsg: 'Delete this weight record?',
    deleteAria: 'Delete',
    pickType: 'Please enter a workout type',
    invalidNumber: 'Please enter a valid number',
    saved: 'Saved'
  },
  anniversary: {
    title: 'Anniversaries',
    add: 'Add anniversary',
    count: 'items',
    name: 'Name',
    namePlaceholder: 'Anniversary name',
    note: 'Note',
    notePlaceholder: 'Optional',
    date: 'Date',
    month: 'Mo',
    save: 'Save',
    cancel: 'Cancel',
    empty: 'No anniversaries yet. Tap + at bottom right to add one.',
    today: 'Today',
    tomorrow: 'Tomorrow',
    daysLeft: '{n} days left',
    nameRequired: 'Name is required',
    dateRequired: 'Please pick a date',
    added: 'Anniversary added',
    confirmDeleteTitle: 'Delete anniversary',
    confirmDelete: 'Delete "{name}"?'
  },
  quote: {
    add: 'Add quote',
    text: 'Quote',
    textPlaceholder: 'Write down a line that moved you…',
    category: 'Category',
    categoryPlaceholder: 'Optional, e.g. Quote/Thought',
    date: 'Date',
    save: 'Save',
    cancel: 'Cancel',
    empty: 'No quotes yet. Tap "Add a quote" above to collect lines you love.',
    textRequired: 'Quote cannot be empty',
    added: 'Quote saved',
    confirmDeleteTitle: 'Delete quote',
    confirmDelete: 'Delete this quote?'
  },
  schedule: {
    agenda: 'Agenda',
    overdue: 'Overdue',
    today: 'Due today',
    upcoming: 'Upcoming',
    dueDate: 'Due date',
    empty: 'No due tasks',
    emptyToday: 'Nothing due today'
  },
  errors: {
    aiUrlCloud: 'Set the cloud endpoint in Settings first',
    aiUrlLocal: 'Set the Ollama endpoint in Settings first',
    aiUrlInvalid: 'Invalid endpoint format; must start with http:// or https://',
    aiProtocolUnsupported: 'Unsupported protocol {protocol}; only http/https allowed',
    aiBlockedHost: 'Endpoint points to a restricted internal address and was blocked by security policy',
    ollamaConnect: 'Ollama connection failed ({status}). Make sure Ollama is running locally',
    cloudFailed: 'Cloud request failed ({status}){detail}',
    cloudUnauthorized: 'Cloud auth failed (401): check your API Key',
    cloudRateLimited: 'Too many requests (429): please try again later',
    idbDisabled: 'The current browser has disabled the local database (IndexedDB); notes cannot be persisted. Check privacy/incognito settings.',
    idbReadFailed: 'Local database read failed; notes could not be loaded',
    removeFolderFailed: 'Failed to delete folder: {msg}',
    createNoteFailed: 'Failed to create note: {msg}',
    saveFailed: 'Save failed: {msg}',
    deleteNoteFailed: 'Failed to delete note: {msg}',
    aiDeleteFailed: 'Failed to delete conversation: {msg}',
    backupNotJson: 'File content is not a valid JSON object',
    backupNotQingjian: 'Not a QingJian backup file (missing app marker)',
    backupNoVersion: 'Backup file is missing a version number',
    backupVersionTooNew: 'Backup version v{version} is newer than the supported v{current}; please upgrade the app',
    backupIncomplete: 'Backup data incomplete (notes / todos missing)',
    backupNoteCorrupt: 'Note data corrupt: an entry is missing an id',
    backupTooLarge: 'Backup file too large (over 50MB); rejected to avoid out-of-memory',
    backupParse: 'File is not valid JSON',
    backupRead: 'File read failed'
  }
}
